...
def run():
    while True:
        ...
        def save_state(page, product):
            with open("Reinigungshilfen/memory.txt", "w") as file:
                file.write(f"{page},{product}")

        def load_state():
            if os.path.exists("Reinigungshilfen/memory.txt"):
                with open("Reinigungshilfen/memory.txt", "r") as file:
                    state = file.read().split(',')
                    return int(state[0]), int(state[1])
            return 1, 0

        with open('Reinigungshilfen/data.csv', 'w') as file:
        ...
        driver.get('https://store.igefa.de/c/reinigungshilfen/moheDPVG3pP8t8kgmxZrDc')
        time.sleep(2)
        ...
        for i in range(start_page, get_last_page):
            driver.get(f'https://store.igefa.de/c/reinigungshilfen/moheDPVG3pP8t8kgmxZrDc?page={i}')
            time.sleep(2)
            ...
            while x < len(elements):
                try:
                    ...
                    with open('Reinigungshilfen/data.csv', 'a') as file:
                        writer = csv.writer(file)
                        writer.writerow((
                            product_name,
