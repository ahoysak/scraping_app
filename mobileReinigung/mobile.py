...
def run():
    while True:
        ...
        def save_state(page, product):
            with open("mobileReinigung/memory.txt", "w") as file:
                file.write(f"{page},{product}")

        def load_state():
            if os.path.exists("mobileReinigung/memory.txt"):
                with open("mobileReinigung/memory.txt", "r") as file:
                    state = file.read().split(',')
                    return int(state[0]), int(state[1])
            return 1, 0

        with open('mobileReinigung/data.csv', 'w') as file:
        ...
        driver.get('https://store.igefa.de/c/mobile-reinigung/MegozpYzDSmdBmCfFW8xCc')
        time.sleep(2)
        ...
        for i in range(start_page, get_last_page):
            driver.get(f'https://store.igefa.de/c/mobile-reinigung/MegozpYzDSmdBmCfFW8xCc?page={i}')
            time.sleep(2)
            ...
            while x < len(elements):
                try:
                    ...
                    with open('mobileReinigung/data.csv', 'a') as file:
                        writer = csv.writer(file)
                        writer.writerow((
                            product_name,
